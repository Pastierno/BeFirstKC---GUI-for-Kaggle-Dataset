...
class DataAnalysisApp(QMainWindow):
    ...
    def plot(self):
        ...
        cat_df = self.transformed_df.select_dtypes(include=['object', 'category'])
        # Plot numeric distributions
        if self.hist_check.isChecked() and not num_df.empty:
            win = QMainWindow(self)
            win.setWindowTitle('Histograms')
        ...
        if self.box_check.isChecked() and not num_df.empty:
            win = QMainWindow(self)
            win.setWindowTitle('Boxplots')
            fig = Figure(figsize=(12, 8))
            canvas = FigureCanvas(fig)
            win.setCentralWidget(canvas)
            ax = fig.add_subplot(111)
            ...
            canvas.draw(); win.show()
        if self.corr_check.isChecked() and not num_df.empty:
            win = QMainWindow(self)
            win.setWindowTitle('Correlation Matrix')
            fig = Figure(figsize=(12, 8))
            canvas = FigureCanvas(fig)
            win.setCentralWidget(canvas)
            ax = fig.add_subplot(111)
            ...
            canvas.draw(); win.show()
        if self.line_check.isChecked() and not num_df.empty:
            win = QMainWindow(self)
            win.setWindowTitle('Line Plots')
            fig = Figure(figsize=(12, 8))
            canvas = FigureCanvas(fig)
            win.setCentralWidget(canvas)
            ax = fig.add_subplot(111)
            ...
            canvas.draw(); win.show()
        # Bar plots: numeric means and categorical counts
        if self.bar_check.isChecked():
            if not num_df.empty:
                win = QMainWindow(self)
                win.setWindowTitle('Bar Plot - Numeric Means')
                fig = Figure(figsize=(12, 8))
                canvas = FigureCanvas(fig)
                win.setCentralWidget(canvas)
                ax = fig.add_subplot(111)
                num_df.mean().plot(kind='bar', ax=ax)
                canvas.draw(); win.show()
            if not cat_df.empty:
                for col in cat_df.columns:
                    win = QMainWindow(self)
                    win.setWindowTitle(f'Bar Plot - {col}')
                    fig = Figure(figsize=(12, 8))
                    canvas = FigureCanvas(fig)
                    win.setCentralWidget(canvas)
                    ax = fig.add_subplot(111)
                    cat_df[col].value_counts().plot(kind='bar', ax=ax)
                    ax.set_title(col)
                    canvas.draw(); win.show()
    ...
    def train_model(self):
        ...
        mdl = {
            ...
            'XGBClassifier': lambda: XGBClassifier(**p, use_label_encoder=False, eval_metric='logloss') if p else XGBClassifier(use_label_encoder=False, eval_metric='logloss'),
            'LGBMClassifier': lambda: LGBMClassifier(**p) if p else LGBMClassifier(),
            'LinearRegression': LinearRegression,
            'RandomForestRegressor': RandomForestRegressor,
            'XGBRegressor': lambda: XGBRegressor(**p) if p else XGBRegressor(),
            'LGBMRegressor': lambda: LGBMRegressor(**p) if p else LGBMRegressor()
        }[mn]()
